...
router.post("/stream", async (req, res) => {
  try {
    ...
    try {
      // Stream AI response
      for await (const chunk of aiAnswerStream(content, conversationHistory || [], parentSummary)) {
        fullResponse += chunk;
      ...
      const { data: aiMessage, error: aiMsgError } = await userClient
        .from("messages")
        .insert({
          chat_id: chatId,
      ...
      res.write(`data: ${JSON.stringify({ 
        type: "done", 
        userMessage,
        aiMessage: aiMessage || { content: fullResponse },
        contextUsed: {
